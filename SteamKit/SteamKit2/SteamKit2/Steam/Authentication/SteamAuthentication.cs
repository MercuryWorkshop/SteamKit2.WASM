...
namespace SteamKit2.Authentication
{
    ...
    public sealed class SteamAuthentication
    {
        ...
        async Task<CAuthentication_GetPasswordRSAPublicKey_Response> GetPasswordRSAPublicKeyAsync( string accountName )
        {
            ...
            new Thread(() =>
            {
                AuthenticationService.GetPasswordRSAPublicKey( request );
            }).Start();

            SteamUnifiedMessages.ServiceMethodResponse<CAuthentication_GetPasswordRSAPublicKey_Response> response = null;
            await Task.Run(async () => {
                // Console.WriteLine("Start");
                // await Task.Delay(4000);
                // Console.WriteLine("After");

                while ( PLCAUTH.protobuf == null ) {
                    Console.WriteLine("waiting for response");
                    await Task.Delay(100);
                }

                Console.WriteLine("got it response");
                response = new SteamUnifiedMessages.ServiceMethodResponse<CAuthentication_GetPasswordRSAPublicKey_Response>( PLCAUTH.protobuf );
                Console.WriteLine("??");
            });
        ...
        public async Task<CredentialsAuthSession> BeginAuthSessionViaCredentialsAsync( AuthSessionDetails details )
        {
            ...
            Console.WriteLine("sending GetPasswordRSAPublicKeyAsync");
            // Encrypt the password
            var publicKey = await GetPasswordRSAPublicKeyAsync( details.Username! );
            Console.WriteLine("GOT PasswordRSAPublicKeyAsync");
            ...
            var encryptedPassword = JsCrypto2.RsaEncrypt(publicKey.publickey_mod, publicKey.publickey_exp, Encoding.UTF8.GetBytes(details.Password));

            Console.WriteLine("PAST ENCRYPTION");
...
public partial class JsCrypto2 {
    [JSImport("encryptrsa", "interop.js")]
    internal static partial byte[] RsaEncrypt(string modulus, string exponent, byte[] data);
}
